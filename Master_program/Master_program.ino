...
int decayTime=100; //sd ray fade out
byte tempo=120;
byte wInstrument=0;
bool Play=false;
bool Record=false;
...
void writeNoteYM(byte note){
    byte block=note/12-1;

    int note_freq[12]={617, 653, 692, 733, 777, 823, 872, 924, 979, 1037, 1099, 1164};
    byte index=note%12;
    digitalWrite(C0, LOW);
    digitalWrite(C1, LOW);
   ...
   //setreg(0xA4, 0x22); // Set frequency
    //setreg(0xA0, 0x20); // Set frequency
    setreg(0xA4, block<<3 | highByte(note_freq[index])); // Set frequency Block then MSB freq for 3 bits
    setreg(0xA0, lowByte(note_freq[index])); // Set frequency
    setreg(0x28, 0xF0); // Key on
}

void releaseNoteYM(byte note){
...
static void write_ym(uint8_t data) {
   //select lines for chip
  digitalWrite(SEL0,(wTrack&1)?HIGH:LOW);
  digitalWrite(SEL1,(wTrack&2)?HIGH:LOW);
  digitalWrite(SEL2,(wTrack&4)?HIGH:LOW);
  digitalWrite(SEL3,(wTrack&8)?HIGH:LOW);
  delayMicroseconds(1);
  digitalWrite(INH2, LOW);
...
void YMregSetup(){
  ...
  setreg(0xB0, 0x30+wInstrument-2); // Feedback/algorithm
